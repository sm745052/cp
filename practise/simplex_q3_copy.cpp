...
public:
	...
	double maximum;
	bool isUnbounded;
	vector<int> basic_variable_list;
...
public:
	Simplex(vector<vector<double>> matrix, vector<double> b, vector<double> c)
	{
		...
		B.resize(b.size());
		C.resize(c.size());
		basic_variable_list.resize(rows, 0);
		ratios.resize(rows, 0);
		for (int i = rows; i < rows + cols; i++)
		{
			basic_variable_list[i - rows] = i;
		}
	...
	{
		double pivetValue = A[pivotRow][pivotColumn];
		...
		basic_variable_list[pivotRow] = pivotColumn;
	}
	...
	void print(char choose)
	{
		...
		if (choose == 'i')
		{
			...
			for (int i = 0; i < cols - rows; i++)
			{
				int flag = 0;
				int ind = 0;
				for (int j = 0; j < basic_variable_list.size(); j++)
				{
					if (basic_variable_list[j] == i)
					{
						flag = 1;
						ind = j;
						break;
		...
		if (choose == 'b')
		{
			...
			for (int i = 0; i < B.size(); i++)
			{
				cout << "x" << basic_variable_list[i] << "=" << B[i] << endl;
			}
			return;
		}
		cout << endl;
...
int main()
{
	...
	while (!res)
	{
		res = simplex.simplexIteration();
	}
	vector<int> basic_variables_list = simplex.basic_variable_list;
	vector<double> basic_variables_values = simplex.B;
	map<int, double> final_values;
	for (int i = 0; i < n; i++)
	{
		if (find(basic_variables_list.begin(), basic_variables_list.end(), i) != basic_variables_list.end())
		{
			final_values[i] = basic_variables_values[find(basic_variables_list.begin(), basic_variables_list.end(), i) - basic_variables_list.begin()];
		}
		else
		{
			final_values[i] = 0;
	...
	ctr = 0;
	for (auto it = final_values.begin(); it != final_values.end(); it++)
	{
		if (find(free_variables.begin(), free_variables.end(), make_pair(it->first, it->first + 1)) != free_variables.end())
		{
			cout << "x" << ctr << "=" << it->second - next(it)->second << endl;
			it++;
			ctr++;
		}
		else
		{
			cout << "x" << ctr << "=" << it->second << endl;
		}
		ctr++;
	}
	return 0;
